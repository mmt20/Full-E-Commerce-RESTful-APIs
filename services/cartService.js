...
exports.addProductToCart = asyncHandler(async (req, res, next) => {
  ...
  // 1) Check if there is a cart for the logged-in user
  let cart = await Cart.findOne({ cartOwner: req.user._id });

  if (cart) {
    // 2) Check if the product already exists in the user's cart by product ID
    ...
    cart = await Cart.create({
      ...
      products: [{ product: productId, price: product.price, quantity: 1 }],
    });
  }
...
exports.updateCartProductCount = asyncHandler(async (req, res, next) => {
  ...
  const cart = await Cart.findOne({ cartOwner: req.user._id })
    ...
    .populate({
      ...
      select: 'title imageCover ratingsAverage  category',
